...
function folderNavigation() {
    function onPostRender(renderCtx) {
        if (renderCtx.rootFolder) {
            ...
            var docLibName = $('table[id*=DoclibViewTbl]').attr('summary');
    ...
    function RenderItems(renderCtx, navigationItems) {
        ...
        if (folderNavDiv == null) {
            ...
            folderNavDiv.setAttribute('class', 'folder-nav-container');
        ...
        for (var index = 0; index < navigationItems.length; index++) {
            if (navigationItems[index].url == '') {
                ...
                span.setAttribute('class', 'folder-nav-node');
            ...
            if (index != navigationItems.length - 1) {
                var span = document.createElement('span');
                span.setAttribute('class', 'folder-nav-separator');
                span.innerHTML = ' > ';
                folderNavDiv.appendChild(span);
            }
        }
...
RegisterModuleInit('/wmp/js/jslink/FolderNavigation.js', folderNavigation);
